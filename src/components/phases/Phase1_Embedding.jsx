...
const Phase1_Embedding = ({
  tokens = [],
  processedVectors = [],
  rawVectors = [], // Need this for base_vector, positional_vector access
  axisMap,
  scenarioId,
  noise, setNoise,
  ...
  const axisConfig = useMemo(() => {
    return {
      x: {
        pos: axisMap?.x_axis?.positive || "Weltbezug",
        neg: axisMap?.x_axis?.negative || "Sprachstruktur",
        desc: axisMap?.x_axis?.description || "Inhaltliche Tiefe vs. syntaktische Form."
      },
      y: {
        pos: axisMap?.y_axis?.positive || "Kontextbindung",
        neg: axisMap?.y_axis?.negative || "Eigenständigkeit",
        desc: axisMap?.y_axis?.description || "Funktionale Rolle im Satzbau."
      }
    };
  }, [axisMap]);
  ...
  const getInspectorData = useCallback((id) => {
    ...
    const rawVector = rawVectors.find(v => Number(v.id) === Number(id) || Number(v.token_index) === Number(id));
  ...
  }, [processedVectors, noise, positionWeight, tokens, rawVectors, axisConfig, getSemanticPosition]);
  ...
  return (
    <PhaseLayout
      ...
      theme={theme}
      badges={[
        { text: `Vektoren: ${tokens.length}`, className: "bg-primary/10 text-primary border-primary/20" },
      ...
      visualization={
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none z-10 px-2 py-2">
            <div className="absolute top-4 left-1/2 -translate-x-1/2 text-[9px] font-black uppercase tracking-widest text-primary/60 flex flex-col items-center">
              <span>↑ {axisConfig.y.pos}</span>
            </div>
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[9px] font-black uppercase tracking-widest text-primary/60 flex flex-col items-center">
              <span>{axisConfig.y.neg} ↓</span>
            </div>
            <div className="absolute left-2 lg:left-4 top-1/2 -rotate-90 -translate-x-1/2 text-[9px] font-black uppercase tracking-widest text-primary/60 whitespace-nowrap">
              ← {axisConfig.x.neg}
            </div>
            <div className="absolute right-2 lg:right-4 top-1/2 rotate-90 translate-x-1/2 text-[9px] font-black uppercase tracking-widest text-primary/60 whitespace-nowrap">
              {axisConfig.x.pos} →
            </div>
          </div>

          <div className="absolute top-6 right-6 flex flex-col gap-2 z-[60]">
            <button onClick={() => zoom(0.2)} className="w-10 h-10 rounded-xl bg-explore-nav border border-explore-border text-content-main hover:bg-primary-hover hover:text-white transition-all shadow-xl font-bold">+</button>
            <button onClick={() => zoom(-0.2)} className="w-10 h-10 rounded-xl bg-explore-nav border border-explore-border text-content-main hover:bg-primary-hover hover:text-white transition-all shadow-xl font-bold">−</button>
          ...
          {showInfo && (
            <div className="absolute top-20 right-6 lg:right-20 z-[60] w-64 p-5 rounded-2xl bg-explore-nav border border-explore-border shadow-2xl backdrop-blur-md animate-in zoom-in-95 duration-200">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-primary mb-2">Vektor-Orientierung</h4>
          ...
          >
            ...
            {processedVectors.map((vec, i) => {
              ...
              return (
                ...
                >
                  <div className={`w-3 h-3 rounded-full transition-all duration-300 border-2 ${isSelected ? 'scale-150 bg-white border-primary shadow-[0_0_15px_var(--color-primary)]' : 'bg-primary-hover border-transparent opacity-60 hover:opacity-100'}`} />
                  <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-explore-nav/90 px-2 py-0.5 rounded text-[10px] text-primary font-bold border border-explore-border whitespace-nowrap pointer-events-none shadow-xl" style={{ transform: `translateX(-50%) scale(${1 / transform.scale})` }}>
                    {token.text}
                  </div>
                </div>
              );
      ...
      controls={[
        <div key="ctrl-noise" className="px-4 py-3 bg-explore-card rounded-xl border border-explore-border shadow-sm">
          <div className="flex justify-between items-center mb-2">
            <label className="text-[8px] uppercase font-black text-primary tracking-widest">Semantic Noise</label>
            <div className="text-[10px] font-mono text-primary font-bold">{noise.toFixed(2)}</div>
          </div>
          <input type="range" min="0" max="5" step="0.1" value={noise} onChange={(e) => setNoise(parseFloat(e.target.value))} className="w-full h-1 bg-explore-border rounded-lg appearance-none cursor-pointer accent-primary" />
        </div>,
        ...
        <div key="ctrl-nav" className="px-4 py-3 bg-explore-card rounded-xl border border-explore-border shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <div className="grid grid-cols-3 gap-0.5 bg-explore-item p-0.5 rounded-lg border border-explore-border">
              <div /> <button onClick={() => move(0, 1)} className="w-6 h-6 flex items-center justify-center bg-explore-nav text-content-main rounded-sm hover:bg-primary hover:text-white text-[8px] transition-colors">▲</button> <div />
              <button onClick={() => move(1, 0)} className="w-6 h-6 flex items-center justify-center bg-explore-nav text-content-main rounded-sm hover:bg-primary hover:text-white text-[8px] transition-colors">◀</button>
              <button onClick={() => move(0, -1)} className="w-6 h-6 flex items-center justify-center bg-explore-nav text-content-main rounded-sm hover:bg-primary hover:text-white text-[8px] transition-colors">▼</button>
              <button onClick={() => move(-1, 0)} className="w-6 h-6 flex items-center justify-center bg-explore-nav text-content-main rounded-sm hover:bg-primary hover:text-white text-[8px] transition-colors">▶</button>
            </div>
            <div className="flex flex-col gap-1 flex-1">
              <button onClick={() => zoom(0.1)} className="h-6 bg-explore-nav text-content-main rounded border border-explore-border hover:bg-primary hover:text-white text-[10px] font-bold transition-colors">+</button>
              <button onClick={() => zoom(-0.1)} className="h-6 bg-explore-nav text-content-main rounded border border-explore-border hover:bg-primary hover:text-white text-[10px] font-bold transition-colors">−</button>
            </div>
          </div>
        </div>
