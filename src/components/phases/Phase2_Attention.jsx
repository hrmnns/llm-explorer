...
const Phase2_Attention = ({ simulator, setHoveredItem, theme }) => {
  ...
  // Extraktion aus dem Simulator (Inkl. Pipeline-Daten)
  const { 
    activeProfileId, 
    setActiveProfileId, 
    processedVectors,
  ...
  const updateInspector = useCallback(() => {
    let rule = activeRules.find(r => Number(r.target) === Number(hoveredTokenId)) || 
               [...activeRules].sort((a, b) => b.strength - a.strength)[0];

    const targetId = rule?.target || hoveredTokenId;
    const targetToken = tokens.find(t => Number(t.id) === Number(targetId));
    
    // PIPELINE-LOGIK: Stärke wird durch Signalqualität beeinflusst
    const rawStrength = rule ? rule.strength : 0;
    const effectiveStrength = rawStrength * pipelineSignal;
    const isLive = !!hoveredTokenId;

    setHoveredItem({
      title: isLive ? `🔍 Attention-Pipeline Analyse` : `🔒 Fokus: ${headDefinitions[activeHead].label}`,
      subtitle: `Signal-Integrität: ${(pipelineSignal * 100).toFixed(0)}%`,
      data: {
        "--- Pipeline Status": "---",
        "QUERY": sourceToken ? `"${sourceToken.text}"` : "N/A",
        "KEY (Target)": targetToken ? `"${targetToken.text}"` : "Kein Fokus",
        "--- Signal-Einfluss": "---",
        "Basis-Gewicht": (rawStrength * 100).toFixed(1) + "%",
        "Effektive Stärke": (effectiveStrength * 100).toFixed(1) + "%",
        "Signal-Dämpfung": ((1 - pipelineSignal) * -100).toFixed(0) + "%",
        "--- Erkenntnis": "---",
        "Information": rule ? rule.explanation : "Keine aktive Verbindung unter den aktuellen Rausch-Bedingungen."
      }
    });
  }, [tokens, activeRules, setHoveredItem, sourceToken, activeHead, hoveredTokenId, pipelineSignal]);

  useEffect(() => {
    updateInspector();
  }, [updateInspector]);
  
  // Dynamische Farbwahl basierend auf Pipeline-Status
  const themeColor = pipelineSignal < 0.4 ? '#ef4444' : (pipelineSignal < 0.7 ? '#f97316' : '#3b82f6');

  return (
    <PhaseLayout
      title="Phase 2: Self-Attention Pipeline"
      subtitle="Relationales Mapping basierend auf Arbeitsvektoren"
      theme={theme}
      badges={[
        { text: headDefinitions[activeHead].label, className: "border-blue-500/30 text-blue-400 bg-blue-500/10" },
        { text: `Signal: ${(pipelineSignal * 100).toFixed(0)}%`, className: pipelineSignal < 0.6 ? "bg-red-500/20 text-red-400 border-red-500/30" : "bg-blue-500/10 text-blue-400" }
      ]}
      visualization={
        <div className="relative w-full h-full min-h-[380px] flex items-center justify-center overflow-hidden bg-slate-950/20 rounded-2xl p-4"
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2 z-50">
            <button onClick={handleZoomIn} className="w-9 h-9 rounded-xl bg-slate-900 border border-white/20 text-white flex items-center justify-center hover:bg-blue-600 shadow-xl">+</button>
            <button onClick={handleZoomOut} className="w-9 h-9 rounded-xl bg-slate-900 border border-white/20 text-white flex items-center justify-center hover:bg-blue-600 shadow-xl">-</button>
          </div>

          <div className="relative w-full max-w-[450px] aspect-square transition-all duration-500 flex items-center justify-center">
            <svg viewBox={`0 0 ${V_SIZE} ${V_SIZE}`} className="absolute inset-0 w-full h-full overflow-visible pointer-events-none z-10">
              ...
              {tokens.map((token, i) => {
                ...
                // PIPELINE-EFFEKT: Linienstärke ist abhängig vom globalen Signal
                const strength = rule ? (rule.strength * pipelineSignal) : 0;
                ...
                return (
                  <g key={`l-${token.id}`}>
                    <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#1e293b" strokeWidth="0.5" strokeDasharray="2 4" />
                    {strength > 0.01 && (
                      <>
                        <line 
                          x1={x1} y1={y1} x2={x2} y2={y2} 
                          stroke={themeColor} 
                          strokeWidth={strength * 12} 
                          opacity={0.2 + strength * 0.6} 
                          style={{ filter: `url(#glowAtt) blur(${(1 - pipelineSignal) * 3}px)` }} 
                          className="transition-all duration-300" 
                        />
                        <circle r={1.5 + strength * 1.5} fill="white">
                          <animateMotion dur={`${2.5 - strength * 2}s`} repeatCount="indefinite" path={`M ${x2} ${y2} L ${x1} ${y1}`} />
                        </circle>
                      </>
                    )}
                  </g>
            ...
            <div className="absolute inset-0 w-full h-full pointer-events-none z-20 overflow-visible">
              {tokens.map((token, i) => {
                ...
                if (isCenter) {
                  tokenStyle = { borderColor: themeColor, backgroundColor: '#0f172a', color: 'white', boxShadow: `0 0 20px ${themeColor}40` };
                } else if (strength > 0.1) {
                  tokenClasses += 'bg-slate-900 border-white text-white scale-110 z-30';
                } else if (hasActivity) {
                  tokenClasses += 'bg-slate-950/80 text-slate-300';
                  tokenStyle = { borderColor: `${themeColor}60` };
                } else {
                  ...
                  <div key={`tp-${token.id}`} className="absolute pointer-events-auto transition-all duration-500 ease-out"
                    ...
                    {isCenter ? (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 flex items-center justify-center bg-slate-900 shadow-2xl transition-all duration-500"
