...
const Phase4_Decoding = ({ simulator, setHoveredItem, theme }) => {
  const { temperature, setTemperature, finalOutputs, activeAttention } = simulator;
  ...
  const getInspectorData = (out, index) => {
    ...
    return {
      ...
      data: {
        ...
        "Status": statusText,
        "--- Risikobewertung": "---",
        "Signalverlust": ((1 - pipelineSignal) * 100).toFixed(0) + "%",
        "Halluzinations-Risiko": out.isCritical ? "HOCH (System-Noise)" : "Normal",
        "--- Information": "---",
        "Kontext": out.causality_trace || `Token-Projektion basierend auf ${out.type}-Aktivierung.`
      }
    };
  };
  ...
  }, [sortedOutputs, selectedLabel, setHoveredItem, topK, minPThreshold, pipelineSignal]);

  return (
    <PhaseLayout
      title="Phase 4: Softmax Decoding Pipeline"
      subtitle="Vom Vektor-Signal zur Wort-Wahrscheinlichkeit"
      theme={theme}
      badges={[
        ...
        { text: `Temp: ${temperature.toFixed(1)}`, className: "border-slate-500/30 text-slate-400 bg-white/5" }
      ]}
      visualization={
        <div className="flex flex-row h-[380px] lg:h-full w-full gap-4 relative" onClick={() => { setSelectedLabel(null); setHoveredItem(null); }}>
          
          {/* PIPELINE OVERLAY BEI CRITICAL NOISE */}
          {isCritical && (
            <div className="absolute inset-0 z-40 bg-red-950/5 pointer-events-none flex items-center justify-center">
              <div className="text-[8px] font-black text-red-500/30 uppercase tracking-[0.5em] rotate-12 select-none">
                High Entropy Interference Detected
              </div>
            </div>
          )}
          ...
          <div className="relative flex-1 h-full flex flex-col justify-end group/chart">
            ...
            <div className="relative flex items-end justify-around gap-1 lg:gap-2 h-full pb-10">
              {sortedOutputs.map((out, i) => {
                ...
                const isAboveThreshold = out.probability >= minPThreshold;
                const isActive = isTopK && isAboveThreshold;
                ...
                return (
                  <div 
                    ...
                    } ${!isActive ? 'opacity-10 grayscale blur-[1px]' : 'opacity-100'}`}
                  ...
                  >
                    ...
                    <div 
                      className={`w-full max-w-[36px] rounded-t-md transition-all duration-700 relative ${
                        isSelected ? 'ring-2 ring-white shadow-2xl' : 'border border-transparent'
                      } ${isCritical && isActive ? 'animate-pulse' : ''}`}
                      style={{ 
                        height: `${out.probability * 85}%`,
                        backgroundColor: isActive ? (colorMap[out.type] || '#475569') : '#1e293b',
                        boxShadow: isWinner && isActive ? `0 0 20px ${colorMap[out.type]}40` : 'none'
                      }}
                    >
                      {/* WINNER SYMBOL */}
                      {isWinner && isActive && (
